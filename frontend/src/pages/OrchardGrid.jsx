      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm mb-6 overflow-x-auto">
        <div className="flex gap-1 mb-1">
          ...
          {Array.from({ length: cols }, (_, c) => (
            <div key={c} className="w-12 text-center text-xs text-gray-400 font-medium">{c + 1}</div>
          ))}
        </div>
        {Array.from({ length: rows }, (_, r) => (
          <div key={r} className="flex gap-1 mb-1 items-center">
            <div className="w-8 text-xs text-gray-400 font-medium text-right pr-1 shrink-0">{r + 1}</div>
            {Array.from({ length: cols }, (_, c) => {
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <h2 className="text-base font-semibold text-gray-900">
                Tree #{selected.id} — Row {selected.rowNumber}, Col {selected.columnNumber}
              </h2>
