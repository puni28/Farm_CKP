...
export default function InspectionList({ treeId, onTreeUpdated }) {
  ...
  const handle = (key) => (e) => setForm(f => ({ ...f, [key]: e.target.type === 'checkbox' ? e.target.checked : e.target.value }));
          ...
          {showForm && (
            <form onSubmit={submit} className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-4 grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <select value={form.status} onChange={handle('status')} className="border border-gray-300 rounded px-2 py-1.5 w-full">
              ...
              <div>
                ...
                <input type="text" value={form.canopyCondition} onChange={handle('canopyCondition')}
                  placeholder="e.g. Dense and green"
                  className="border border-gray-300 rounded px-2 py-1.5 w-full" />
              </div>
              <div>
                <label className="block text-xs text-gray-600 mb-1">Trunk Condition</label>
                <input type="text" value={form.trunkCondition} onChange={handle('trunkCondition')}
                  ...
                  {inspections.map(ins => (
                    <tr key={ins.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-2 pr-3">
                        {[ins.diseaseObserved && '🦠 Disease', ins.pestObserved && '🐛 Pest', ins.irrigationIssue && '💧 Irrigation'].filter(Boolean).join(' ') || '—'}
                      </td>
